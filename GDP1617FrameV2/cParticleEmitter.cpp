	...
	bool bOverwriteExisting /*=true*/)
{
	...
	for (int count = 0; count != numberOfParticles; count++)
	{
		...
		this->m_vecParticlesCOPY.push_back(tempParticle);
	}
...
cParticleEmitter::cParticleEmitter()
{
	...
	this->bKeepThreadAlive = true;

	InitializeCriticalSection(&CSParticleUpdate);

	return;
}


void cParticleEmitter::Lock(void)
{
	EnterCriticalSection(&CSParticleUpdate);
	this->m_bIsLocked = true;
	return;
}


void cParticleEmitter::UnLock(void)
{
	this->m_bIsLocked = false;
	LeaveCriticalSection(&CSParticleUpdate);
	return;
}

bool cParticleEmitter::bIsLocked(void)
{
	return this->m_bIsLocked;
}

void cParticleEmitter::updateParticleCopy(void)
{
	int numParticles = this->m_vecParticles.size();

	for (int index = 0; index != numParticles; index++)
	{
		this->m_vecParticlesCOPY[index] = this->m_vecParticles[index];
	}
...
{
	// Find centre of all the particles
	glm::vec3 centre;
	for (int count = 0; count != this->m_vecParticles.size(); count++)
	{
		centre += this->m_vecParticles[count].position;
	}
	centre /= this->m_vecParticles.size();
	...
	for (int index = 0; index != this->m_vecParticles.size(); index++)
	{
		...
		{	// Update it as it's alive
			...
			// 
			glm::vec3 vecToCentre = centre - this->m_vecParticles[index].position;

			const float SomeRatio = 0.1f;
			this->m_vecParticles[index].acceleration = vecToCentre * SomeRatio;
...
int cParticleEmitter::getParticlesCopy(std::vector<cParticle> &vecParticles)
{
	...
	{	// It's not, so allocate and resize
		...
		unsigned int particleToCreate
			= static_cast<unsigned int>(this->m_vecParticlesCOPY.size() - vecParticles.size());
	...
	for (unsigned int index = 0; index != vecParticles.size(); index++)
	{
		...
		if (this->m_vecParticlesCOPY[index].life > 0.0f)
		{
			vecParticles[indexReturned] = this->m_vecParticlesCOPY[index];
			indexReturned++;
		}
	}
